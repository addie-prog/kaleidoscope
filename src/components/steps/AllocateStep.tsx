...
export default function AllocatePage({ onNext, selectedValues, Principles, reportData, userNotes, updatedPrinciples, ResetPrinciples, project }: props) {
  ...
  const ValidationIcon = (color: string, margin: string = "0px") => {
    return <svg className={margin ? `mt-[${margin}]` : ""} width="15" height="15" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div
            className={`
              ...
              ${showFloatingStatus && !showToast
                ? "fixed sm:right-6 md:right-6 right-2 top-4"
                : "relative hidden"}
            `}
          >
            {AllocateMessage()}
