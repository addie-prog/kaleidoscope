...
export async function GET(req: Request) {
  ...
  let query = adminDb.collection("interactions").orderBy("Date Created", "desc").limit(pageSize);

  if (pageToken) {
    const lastDocSnap = await adminDb.collection("interactions").doc(pageToken).get();
