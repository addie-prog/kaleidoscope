...
export default function Dashboard2Page({
  ...
  const [currentCards, setActiveCards] = useState<Array<any>>([]);
  ...
  const getCardsForSubTab = (subCardId: string): any => {
    ...
    const groupedCards = (cardsMap[subCardId] || []).reduce(
      (acc: any, card: any) => {
        const category =
  ...
  const toggleCheckbox = (cardId: number, stepId: number, category: any) => {
    ...
    setActiveCards((prev) => ({
      ...prev,
      [category]: prev[category]?.map((card: any) =>
        card.id === cardId
          ? {
            ...card,
            steps: card.steps.map((step: any) =>
  ...
  useEffect(() => {
    setActiveCards(getCardsForSubTab(activeSubTab))
  }, [activeSubTab])
  ...
  const toggleSkip = (cardId: number, stepId: number, categoryKey: any) => {
    ...
    setActiveCards((prev) => ({
      ...prev,
      [categoryKey]: prev[categoryKey]?.map((card: any) =>
        card.id === cardId
          ? {
            ...card,
            steps: card.steps.map((step: any) =>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F0F0F0]">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar */}
        <aside
          className={`w-72 bg-white border-r border-[#E5E5E5] flex flex-col z-40 transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
            }`}
        >
          {/* Scrollable content */}
...
/></div>
                ...
                <div className="space-y-0 border-t border-[#E5E7EB] pb-2">
                  {tabs?.filter((t: objectType)=>t.checked === true )?.map((tb: objectType, ij: number)=>
                   { 
                    ...
                    return <div key={ij} className={`flex justify-between items-center py-2 px-4 ${Number(totalTabs)-1 > ij ? "border-b" : "" } border-[#E5E7EB]`}>
                    <div className="flex items-center gap-1.5">
                      <div className="w-1.5 h-1.5 rounded-full" style={{color: 
                        tb.color
                      }} />
          ...
          <div className=" bottom-0 bg-white border-t border-[#E5E5E5] p-4 space-y-2">
            <div className="flex gap-2.5">
              ...
              <button onClick={saveProgressModal.openModal} className="flex-1 flex items-center justify-center gap-1 h-11 px-2.5 py-3 border border-[#E5E7EB] bg-[#F9FAFB] rounded-xl hover:bg-gray-100 transition-colors">
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
            ...
            <button onClick={downloadReportModal.openModal} className="w-full flex items-center justify-center gap-1 h-11 px-2.5 py-3 bg-[#3B82F6] rounded-lg hover:bg-[#2563EB] transition-colors">
              <svg
                width="20"
                height="20"
                viewBox="0 0 20 20"
            ...
            <button onClick={downloadCSVModal.openModal} className="w-full flex items-center justify-center gap-1 h-11 px-2.5 py-3 bg-[#3B82F6] rounded-lg hover:bg-[#2563EB] transition-colors">
              <svg
                width="20"
                height="20"
                viewBox="0 0 20 20"
              ...
              <span className="text-base font-semibold text-white">Download CSV</span>
            </button>
          </div>
        </aside>
        ...
        <main className="flex-1 flex flex-col w-full lg:w-auto overflow-x-hidden m-5">
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar bg-white p-5">
              ...
              {currentSubTabs?.map((subTab: any) => {
                ...
                return (
                  <button
                    ...
                    className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg whitespace-nowrap transition-colors ${isActiveSubTab
                      ? " text-white"
                      : "border border-[#918D8D] text-[#918D8D] hover:bg-gray-50"
                      }`}
                    style={{ background: isActiveSubTab ? tabs?.filter((p) => p.id == activeTab)[0]?.color : "" }}
                  >
                    <span className="text-base font-medium">{subTab.name}</span>
            ...
            {Object.entries(currentCards)?.length > 0 ?
              Object.entries(currentCards).map(([categoryKey, cards]: any) => (
                ...
                <div key={categoryKey}>
...

                  {
                    (categoryKey == "skip"
                      ?
                      <div className="inline-flex items-center px-3 py-2 rounded-[20px] bg-[#EF4444] my-5">
                        <span className="text-xs font-medium text-white">SKIP FOR NOW ({cards?.length})</span>
                      </div> :
                      categoryKey == "must_have" ?
                        <div className="inline-flex items-center px-3 py-2 rounded-[20px] bg-[#10B981] my-5">
                          <span className="text-xs font-medium text-white">MUST HAVE ({cards?.length})</span>
                        </div>

                        :
                        <div className="inline-flex items-center px-3 py-2 rounded-[20px] bg-[#F59E0B] my-5">
                          <span className="text-xs font-medium text-white">SHOULD HAVE ({cards?.length})</span>
                        </div>)
                  }
                            ...
                            {activeCardTab === "context" && (
                              <div className="px-5 py-7 flex flex-col gap-5">
                                ...
                                <div className="flex w-[80%] gap-3">
                                  {card["Red Flags"] ?
                                    <div className="flex w-[50%] p-3 rounded-md border-2 border-[#EF4444] bg-[#FDF4F4]">
                                      <div className="flex flex-col gap-2">
                                  ...
                                  {card["Green Flags"] ?
                                    <div className="flex w-[50%] p-3 rounded-md border-2 border-[#10B981] bg-[#F4FBF7]">
                                      <div className="flex flex-col gap-2">
              ...
              <div className="py-5 px-6 flex flex-col items-center justify-center">
                ...
                {/* Action Button */}
                <button onClick={() => redirect(`/calculator?project=${project}&step=2`)} className="flex items-center gap-2 px-6 py-3 bg-blue-500 hover:bg-blue-600 text-white font-semibold text-base rounded-lg transition-colors">
                  <svg
                    width="20"
                    height="20"
                    viewBox="0 0 20 20"
