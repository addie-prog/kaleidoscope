...
export default function Dashboard2Page() {
  ...
  const getSubTabsForTab = (tabId: string): SubTab[] => {
    const subTabsMap: Record<string, SubTab[]> = {
      fairness: [
        { id: "data-quality", label: "Data Quality & Representation", percentage: 21 },
        { id: "algorithmic", label: "Algorithmic Testing", percentage: 13 },
        { id: "inclusive", label: "Inclusive Design", percentage: 40 },
        { id: "remediation", label: "Remediation & Accountability", percentage: 15 },
      ],
      privacy: [
        { id: "data-protection", label: "Data Protection", percentage: 30 },
        { id: "encryption", label: "Encryption Standards", percentage: 25 },
        { id: "compliance", label: "Privacy Compliance", percentage: 45 },
      ],
      safety: [
        { id: "security-audit", label: "Security Audit", percentage: 0 },
        { id: "vulnerability", label: "Vulnerability Testing", percentage: 0 },
      ],
      transparency: [
        { id: "documentation", label: "Documentation", percentage: 0 },
        { id: "reporting", label: "Reporting Standards", percentage: 0 },
      ],
      accountability: [
        { id: "governance", label: "Governance Framework", percentage: 0 },
        { id: "monitoring", label: "Monitoring Systems", percentage: 0 },
      ],
      accessibility: [
        { id: "wcag", label: "WCAG Compliance", percentage: 0 },
        { id: "assistive", label: "Assistive Technology", percentage: 0 },
      ],
    };
  ...
  const getCardsForSubTab = (subTabId: string): TaskCard[] => {
    const cardsMap: Record<string, TaskCard[]> = {
      "data-quality": [
        {
          id: 1,
          title: "Bias Audit for Training Data",
      ...
      algorithmic: [
        {
          id: 2,
          title: "Algorithmic Fairness Testing Suite",
      ...
      remediation: [
        {
          id: 4,
          title: "Remediation Framework Implementation",
    ...
    return cardsMap[subTabId] || [];
  };

  const toggleCheckbox = (cardId: number, stepId: number) => {
    setCardSteps((prev) => ({
      ...prev,
      [cardId]: prev[cardId].map((step) =>
        step.id === stepId && !step.skipped
  ...
  const currentSubTabs = getSubTabsForTab(activeTab);
  const currentCards = getCardsForSubTab(activeSubTab);
  ...
  const toggleSkip = (cardId: number, stepId: number) => {
    setCardSteps((prev) => ({
      ...prev,
      [cardId]: prev[cardId].map((step) => {
        if (step.id === stepId) {
  ...
  const saveNote = () => {
    if (activeNoteId) {
      setCardSteps((prev) => ({
        ...prev,
        [activeNoteId.cardId]: prev[activeNoteId.cardId].map((step) =>
          step.id === activeNoteId.stepId ? { ...step, note: tempNote } : step
        ),
      }));
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F0F0F0]">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar */}
        <aside
          className={`fixed lg:static w-72 bg-white border-r border-[#E5E5E5] flex flex-col z-40 transition-transform duration-300 h-[calc(100vh-64px)] ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
            }`}
        >
          {/* Scrollable content */}
            ...
            <div>
              ...
              <div style={{
                ...
                <div className="space-y-0 border-t border-[#E5E7EB] pb-2">
                  ...
                  <div className="flex justify-between items-center py-2 px-4 border-[#E5E7EB]">
                    <div className="flex items-center gap-1.5">
          ...
          <div className="sticky bottom-0 bg-white border-t border-[#E5E5E5] p-4 space-y-2">
            <div className="flex gap-2.5">
              <button className="flex-1 flex items-center justify-center gap-1 h-11 px-2.5 py-3 border border-[#E5E7EB] bg-[#F9FAFB] rounded-xl hover:bg-gray-100 transition-colors">
                ...
                <span className="text-base font-semibold text-[#3B82F6]">Edit</span>
              </button>
              <button className="flex-1 flex items-center justify-center gap-1 h-11 px-2.5 py-3 border border-[#E5E7EB] bg-[#F9FAFB] rounded-xl hover:bg-gray-100 transition-colors">
                <svg
                  width="20"
                ...
                <span className="text-base font-semibold text-[#3B82F6]">Save</span>
              </button>
            </div>
            <button className="w-full flex items-center justify-center gap-1 h-11 px-2.5 py-3 bg-[#3B82F6] rounded-lg hover:bg-[#2563EB] transition-colors">
              <svg
                width="20"
                height="20"
                viewBox="0 0 20 20"
        ...
        <main className="flex-1 flex flex-col w-full lg:w-auto overflow-x-hidden m-5">
            ...
            {tabs.map((tab, index) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex-1 flex-col px-6 py-[12px] justify-center items-center gap-1.5 transition-colors border-r border-[#EDEDED] flex-shrink-0 min-w-fit ${isActive
                    ? "bg-white border-0"
                ...
                >
                  <div className="flex items-center gap-2 justify-center">
                    <div className="flex items-center gap-1.5">
                      ...
                      />
                      <span
                        className={`text-base whitespace-nowrap ${isActive ? "font-semibold" : "font-medium"
                          }`}
          ...
          {/* Content Area */}
          <div className="flex-1 overflow-y-auto no-scrollbar bg-white p-5">
            {/* Sub-tabs */}
              ...
              {currentSubTabs.map((subTab) => {
                ...
                return (
                  <button
                    ...
                    className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg whitespace-nowrap transition-colors ${isActiveSubTab
                      ? " text-white"
                      : "border border-[#918D8D] text-[#918D8D] hover:bg-gray-50"
                      }`}
                    style={{ background: isActiveSubTab ? tabs?.filter((p) => p.id == activeTab)[0]?.color : "" }}
                  >
                    <span className="text-base font-medium">{subTab.label}</span>
                    <span className="text-base font-medium">{subTab.percentage}%</span>
                  </button>
                );
              })}
            </div>
            ...
            <div className="inline-flex items-center px-3 py-2 bg-[#10B981] rounded-[20px] mb-5">
              <span className="text-sm font-medium text-white">Must Have (1)</span>
            </div>
                      ...
                      <div>
                        ...
                        <div className="h-1.5 bg-[#F0F0F0] rounded-[10px] overflow-hidden">
                          <div
                            ...
                            style={{ width: `${progressPercentage}%`, background:  tabs?.filter((p) => p.id == activeTab)[0]
                        ?.color}}
                          />
                        </div>
                      </div>
                    ...
                    >
                      ...
                      <div className="flex items-center border-t border-b border-[#E5E5E5] px-5">
                        <button
                          ...
                          className={`flex items-center justify-center gap-1 px-4 py-3 transition-colors ${activeCardTab === "action"
                              ? "border-b-2"
                              : "border-b-2 border-transparent"
                            }`}

                            style={{
                      borderColor: activeCardTab === "action" ? tabs?.filter((p) => p.id == activeTab)[0]
                        ?.color : "",
                    }}
                        >
                          <svg
                        ...
                        ?.color
                          ...
                          <span className={`text-sm font-medium`}
                            style={{
                      color: activeCardTab === "action" ? tabs?.filter((p) => p.id == activeTab)[0]
                        ?.color : "",
                    }}
                            >
                            Action
                        ...
                        <button
                          ...
                          ${activeCardTab === "context"
                            ...
                            style={{
                      borderColor: activeCardTab === "context" ? tabs?.filter((p) => p.id == activeTab)[0]
                        ?.color : "",
                    }}
                        >
                          <svg
                        ...
                        ?.color
                          ...
                          <span className={`text-sm font-medium`}
                            style={{
                      color: activeCardTab === "context" ? tabs?.filter((p) => p.id == activeTab)[0]
                        ?.color : "#323743",
                    }}
                            >
                            Context & Education
                        ...
                        <button
                          onClick={() => setActiveCardTab("diligence")}
                          className={`flex items-center justify-center gap-1 px-4 py-3 transition-colors ${activeCardTab === "diligence" ? "border-b-2"
                              : "border-b-2 border-transparent"
                            }`}
                             style={{
                      borderColor: activeCardTab === "diligence" ? tabs?.filter((p) => p.id == activeTab)[0]
                        ?.color : "",
                    }}
                        >
                          <svg
                          ...
                          >
                            <path
                              ...
                              stroke={`${activeCardTab === "diligence"
                              ? tabs?.filter((p) => p.id == activeTab)[0]
                        ?.color
                              : "#323743"
                            }`}
                        ...
                        ?.color
                          ...
                          <span className={`text-sm font-medium`}
                            style={{
                      color: activeCardTab === "diligence" ? tabs?.filter((p) => p.id == activeTab)[0]
                        ?.color : "#323743",
                    }}
                            >
                            Diligence Tab
                      ...
                      {activeCardTab === "action" && (
                        <div className="px-5 py-7 flex flex-col gap-5">
                          ...
                          <div className="flex flex-col gap-4">
                            <h4 className="text-base font-medium text-[#323743]">
                              Action Steps
                            </h4>
                            <div className="flex flex-col gap-3">
                              {card.steps.map((step, index) => (
                                <React.Fragment key={`${step.id}_${index}`}>
                                  <div className="flex flex-col gap-2">
                                    <div className="flex justify-between items-center">
                                      <div className="flex items-center gap-2">
                                        <button
                                          onClick={() =>
                                            toggleCheckbox(card.id, step.id)
                                          }
                                          ...
                                          style={
    {
      ...
      '--border-color': step.completed ?  tabs?.filter((p) => p.id == activeTab)[0]
                        ?.color : "",
                                        ...
                                        <span
                                          className={`text-base font-normal transition-all ${step.skipped
                                              ? "line-through"
                                              : "text-[#323743]"
                                            }`}
                                            style={{
                      ...
                      color:tabs?.filter((p) => p.id == activeTab)[0]
                        ?.color ,
                                        ...
                                        <button
                                          onClick={() =>
                                            openNote(card.id, step.id, step.note)
                                          }
                                           style={{
                      backgroundColor:tabs?.filter((p) => p.id == activeTab)[0]
                        ?.background ,
      ...
      '--focus-border': tabs?.filter((p) => p.id == activeTab)[0]
                        ?.color,
                                          ...
                                          <div className="flex items-center gap-2">
                                            <button
                                              onClick={saveNote}
                                              style={{background: tabs?.filter((p) => p.id == activeTab)[0]
                        ?.color}}
                                              className="px-4 py-1.5 text-[#fff] text-base font-normal rounded-lg hover:bg-[#7a4dd6] transition-colors"
                                            >
                                              Save
                                            </button>
                                            <button
                                              ...
                                              style={{background: tabs?.filter((p) => p.id == activeTab)[0]
                        ?.color}}
                                              className="px-4 py-1.5 border text-[#fff] text-base font-normal rounded-lg hover:bg-[rgba(137,90,246,0.1)] transition-colors"
                                            >
                                              Cancel
                                            </button>
                                          </div>
                      ...
                      {activeCardTab === "diligence" && (
                        <div className="px-5 py-7">
                          <p className="text-base text-[#323743]">
                            Diligence Tab content goes here...
                          </p>
                        </div>
                      )}
                    </div>
