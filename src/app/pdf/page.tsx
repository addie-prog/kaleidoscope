...
export default function ReportPage() {
    ...
    useEffect(() => {
        const init = () => {
            const selectedValues: objectType = JSON.parse(localStorage.getItem("selectedValues") || "{}");
            const principles: objectType[] = JSON.parse(localStorage.getItem("principles") || "[]");
            const cards: objectType[] = JSON.parse(localStorage.getItem("currentCards") || "{}");
    ...
    useEffect(() => { console.log("principles?.map((p)=>{return p.budget})", principles?.map((p) => { return p.color })) }, [principles])
    ...
    return (
        <div>
          ...
          <div className="w-full flex-row flex justify-right">
            ...
            <main className="mx-auto max-w-4xl px-4 mt-[15px] " ref={contentRef}>
                ...
                {/* Report Header Card */}
                <div
                    className="mb-6 px-4 sm:px-15 sm:py-7 py-6 rounded-lg bg-white"
                    style={{
                    ...
                    <div className="flex flex-col items-center justify-center gap-6 sm:gap-5">
                        ...
                        <div className="flex flex-wrap items-center justify-around  w-full sm:flex-row flex-col gap-[20px]">
                            <div className="flex flex-col items-center gap-2 sm:w-[30%]">
                                ...
                                <div className="md:text-[17px] text-[13px] font-bold text-[#111827] text-center w-full">${topValues["Total cash"]}</div>
                            </div>
                            <div className="flex flex-col items-center gap-2 sm:w-[30%]">
                ...
                <div className="mb-6 flex flex-col items-center gap-6 sm:gap-5 w-full rounded-lg border-[3px] border-gray-200 bg-white sm:py-7 py-6">
                ...
                <div className="mb-6 w-full rounded-lg border-[3px] border-gray-200 bg-white p-5">
                    ...
                    <div className="flex flex-col gap-3">
                        {principles.map((principle) => (
                            <div
                                ...
                                className={`flex gap-0 bg-[${principle.bgcolor}] overflow-hidden`}
                            >
                                <div
                                ...
                                <div className="flex flex-col w-full">
                                        ...
                                        <div className="flex flex-col items-end gap-0">
                                            ...
                                            <span className="text-sm font-medium text-gray-900">
                                                {principle.budget >= 1000
                                                    ? `$${(principle.budget / 1000).toFixed(1)}K`
                                                    : `$${principle.budget}`}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    {/* Nested Layers */}
                                    {principle.layers && principle.layers.length > 0 && (
                                        <div className="flex flex-col gap-2 px-4 pb-3">
                                            {principle.layers.filter((k: objectType) => k.checked == true).map((layer: objectType) => (
                                                <div
                                                    key={layer.id}
                                                ...
                                                >
                                                    ...
                                                    <span className="text-sm font-medium text-gray-900">
                                                        ${layer.budget.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 2 })}
                                                    </span>
                                                </div>
                                            ))}
                                        </div>
                ...
                {Object.entries(items)?.map(([categoryKey, cards]: any) =>
                (Object.values(cards)?.map((cardArr: any) =>
                    cardArr?.map((card: objectType, index: number) =>
                    (<>
                        <div className="mb-6 border-l-2" key={index} style={{
                        ...
                        {/* Main Content */}
                        <div className="mb-6 flex-1 space-y-6 pl-4 pt-4 border-l-2 " style={{
                            borderColor: principles?.filter((p: objectType) => {
                                return p.id == card["Principle"]
                            })[0]?.color
                        }}>
